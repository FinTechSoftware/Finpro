import { CSSResultGroup, html, TemplateResult, PropertyValues } from 'lit';
...
import style from './fp-pagination.css';
...
export default class FpPagination extends FinproElement {
   ...
   */
  @event('fp-change') private onChange: EventDispatcher<{
    selectedPage: number;
    prevPage: number;
    itemsPerPage: number;
  }>;
  ...
  render(): TemplateResult {
    const selectEl = this.hasSelect
      ? html`
          <div class="select">
            ...
            <fp-select @fp-select="${this._selectHandler}">
              ${this.itemsPerPageOptions.map(option => {
                return html`<fp-select-option
                  value="${option.value}"
                  ?selected=${option.value === this.itemsPerPage}
                  >${option.text}</fp-select-option
                >`;
              })}
            </fp-select>
          </div>
    ...
    const jumperEl = this.hasJumper
      ? html` <div class="jumper">
          ...
          <fp-input value="${this.currentPage}" @fp-change="${this._inputHandler}"></fp-input>
        </div>`
      : null;
