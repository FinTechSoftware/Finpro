...
import style from './fp-popover.css';
...
export default class FpPopover extends FinproElement {
   ...
   */
  @event('fp-popover-show') private onFpPopoverShow: EventDispatcher<string>;

  /**
   * Fires when popover becomes hidden
   */
  @event('fp-popover-hide') private onFpPopoverHide: EventDispatcher<string>;
  ...
  set target(value: string | Element) {
    if (typeof value === 'string') {
      ...
      console.warn(
        'FpPopover target only accepts an Element instance or a string id of a DOM element.'
      );
    }
  }
